window.onload = function() {
  ...
  modal =
    '<div class="modal-overlay request-modal"><div class="modal-container"><div class="modal-header"><h3>Register Your Prestashop Store Url</h3></div><div class="modal-body"><div id="wk_info">If your store setup contains any folder then add that folder name like - ex. https://www.example.com/folder_name/</div><div class="input-block"><div class="input-label" <label>Store Url</label></div><div class="block-input"><input type="text" id="url" name="url" value="" /></div></div><div class="modal-footer"><button id="cancel-request-modal" class="cancel" type="button">Cancel</button><button id="request-submit" class="submit" type="button">Next</button></div></div></div>';
          ...
          if (attrHtml.length) {
            ...
            if (productData != undefined) {
              ...
              $.each(productData.skuModule.skuPriceList, function(ind, val) {
                ...
                //console.log(productData.skuModule.productSKUPropertyList);
                ...
                if (val.skuAttr != undefined) {
                  $.each(val.skuAttr.split(";"), function(indx, valu) {
                    $.each(
                      ...
                      function(ind2, val2) {
                        if(val2.skuPropertyName.toLowerCase() == "ships from"){
                          console.log(2);
                          return;
                        }
              ...
              $.each(productData.skuModule.productSKUPropertyList, function(
                ...
                if(val4.skuPropertyName.toLowerCase() == "ships from"){
                  console.log(4);
                  return;
                }
                ...
                $.each(val4.skuPropertyValues, function(ind5, val5) {
                  ...
                  //console.log(val4.skuPropertyName.toLowerCase())
  ...
  var checkAttribute = function() {
    return new Promise(function(resolve, reject) {
      ...
      /*postData( url + "module/democontrollertabs/importsuperattributes", { token: localStorage.aliexpress_import_token,
        ajax: 1,
      ...
      $.ajax({
        url: url + "module/democontrollertabs/importsuperattributes",
        
        data: {
      
          token: localStorage.aliexpress_import_token,
          ajax: 1,
  ...
  var addProductGeneral = function() {
    ...
    return new Promise(function(resolve, reject) {
      $.ajax({
        url: url + "module/democontrollertabs/importproduct",
        data: {
          name: product_name,
          qty: product_qty,
          id: product_id,
  ...
  var addingProductOption = function(product_id, combination) {
    return new Promise(function(resolve, reject) {
      $.ajax({
        url: url + "module/democontrollertabs/importvariation",
        data: {
    ...
    return new Promise(function(resolve, reject) {
      if (
        ...
        /*$.ajax({
          url:
            url +
            "modules/authenticate-user?username=" +
            username +
            "&token=" +
            token,
          dataType: "jsonp",
  ...
  var validateUrl = function() {
    return new Promise(function(resolve, reject) {
      if (url == undefined || url == "") {
        ...
        $.ajax({
          //validate-url
          url: url + "module/democontrollertabs/security?ajax=true",
          dataType: "jsonp",
          jsonp: "callback",
  ...
  $(document).ready(function() {
    ...
    $("body").on("click", "#login-submit", function() {
      ...
      localStorage.aliexpress_import_username = 'test';
                  ...
                  if (sku_attr != "") {
                    ...
                    $.each(curr_sku_attr, function(index, value) {
                      if (value != 0) {
                        ...
                        console.log(window.runParams.data.skuModule
                        ...
                        $.each(
                          ...
                          function(ind1, val1) {
                            console.log(val1);
                            if(val1.skuPropertyName.toLowerCase() == "ships from"){
                              console.log(1);
                              return;
                            }
